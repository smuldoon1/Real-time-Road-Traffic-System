...
public class RoadNetworkEditor : Editor
{
    ...
    // Allows a new road network to be created by clicking the option in the menu bar
    [MenuItem("Traffic System/New Road Network")]
    private static void CreateNewRoadNetwork()
    {
        RoadNetwork rn = new GameObject("Road Network").AddComponent<RoadNetwork>();
        Selection.activeGameObject = rn.gameObject;
        rn.InitialiseRoadNetwork(defaultMaterial);
    }
